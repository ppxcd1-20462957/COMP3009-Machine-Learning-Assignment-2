    ...
    def train(self, X_train, Y_train, X_test, Y_test):
        ...
        for(train, test) in kf.split(inputs, targets):
            ...
            model = models.Sequential([
                                       layers.Dense(128, input_shape = (47, ), activation = activation),
                                       layers.Dropout(0.1),
            ...
            history = model.fit(inputs[train], targets[train], epochs = 1)
...
class Classification_XGBoost:
    ...
    def train(self, X_train, Y_train, X_test, Y_test):
        ...
        return '5-Fold Cross-Validation Accuracy: {}%, Test Score: {}%'.format(round(np.mean(scores['train_score']) * 100, 2), round(np.mean(scores['test_score']) * 100, 2))
...
class Regression_RandomForest:
    ...
    def train(self, X_train, Y_train, X_test, Y_test):
        ...
        model = RandomForestRegressor(criterion = 'absolute_error', max_leaf_nodes = 4, random_state = 42)
        scores = cross_validate(model, inputs, targets, cv = 5, scoring = ['r2', 'neg_mean_absolute_error'], return_train_score = True)
...
class Run_Regression:
    def __init__(self):
        ...
        print('Regression: XGBoost ', xgb.accuracy)
